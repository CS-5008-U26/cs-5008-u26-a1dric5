...
Returns:
dNode *         the new node, or NULL if data was NULL or malloc failed

Notes:
The node is on the heap.  Whoever removes it from a list must free it. */

dNode *makeNode (void *data) {
    if (data == NULL) {
        return (NULL);
    }
...
Parameters:
dNode *list         the current head (may be NULL for an empty list)
dNode *node         the node to add

Returns:
dNode *             the new head of the list */

dNode *addToFront (dNode *list, dNode *node) {
    if (node == NULL) {
...
Returns:
dNode *         that node, or NULL if n is out of range */

dNode *getNth (dNode *list, int n) {
    if (n < 1) {
        return (NULL);
    }
...
Parameters:
dNode *list         the head of the list
dNode *node         the node to remove

Returns:
dNode *             the head of the list, which changes if node WAS the head
...
dNode *deleteNode (dNode *list, dNode *node) {
...
Returns:
dNode *             the new head (which was the old tail)
...
dNode *reverseList (dNode *list) {
    ...
    while (current != NULL) {
        dNode *nextOne = current->next;     // save it before overwriting it
...
Returns:
dNode *             the head of the new list, or NULL on failure
...
void freeList (dNode *list) {
    while (list != NULL) {
        dNode *nextOne = list->next;        // save it before we free the node
        free (list->data);
        free (list);
        list = nextOne;
    }
...
int main (int argc, char **argv) {
    ...
    dNode *list = readCities (filename);
    if (list == NULL) {
