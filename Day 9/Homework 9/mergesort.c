/* This file implements Merge Sort using the provided sort template.
 */
...
/* merge        merge two adjacent sorted runs into one sorted run

Parameters:
a               the array being sorted
left            first index of the left run
mid             last index of the left run; the right run is mid+1..right
right           last index of the right run
temp            scratch array at least as large as a

Returns:
nothing         a[left..right] is left in ascending order


Walk both runs at once, always copying the smaller of the two front
values into temp, then copy temp back over a[left..right].

Using <= rather than < when both sides tie means a value from the LEFT
run is taken first, which is what makes Merge Sort stable.*/

void merge (int a[], int left, int mid, int right, int temp[]) {
    int i = left;                           // walks the left run
    int j = mid + 1;                        // walks the right run
    int k = left;                           // walks temp
    ...
    while (i <= mid) {                      // drain whatever is left over
        temp[k++] = a[i++];
    }
    while (j <= right) {
        temp[k++] = a[j++];
    ...
    for (int x=left; x<=right; x++) {       // copy the merged run back
        a[x] = temp[x];
    }
}

/* mergesortrange       recursively merge sort one slice of the array
...
Split the slice in half, sort each half, then merge them.  A slice of
one element is already sorted, which is where the recursion bottoms out.

mid is computed as left + (right-left)/2 rather than (left+right)/2.
The obvious version can overflow int when both indices are large.*/

void mergesortrange (int a[], int left, int right, int temp[]) {
    if (left < right) {
        int mid = left + (right - left) / 2;
        mergesortrange (a, left, mid, temp);
        mergesortrange (a, mid+1, right, temp);
 ...
 *
 * Notes:
 *   The array is split in half log n times, and each level of splitting
 *   does O(n) work to merge, so the total is O(n log n) -- and unlike
 *   QuickSort, that bound holds for EVERY input.  Merge Sort does not care
 *   whether the data arrives random, sorted, or reversed.
 *
 *   The price is memory: Merge Sort needs O(n) of scratch space.  We
 *   allocate it ONCE here rather than inside merge(), because a malloc and
 *   free on every one of the n-1 merges would dominate the running time.
 */
