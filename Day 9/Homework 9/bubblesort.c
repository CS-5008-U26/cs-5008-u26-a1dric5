/* This file implements bubble sort using the provided sort template.
The main function runs the built-in test cases, then runs three
interactive timing loops: random data, sorted data, and reverse
sorted data. */
...
/* sortarray    sort an array of ints into ascending order. This is a bubble sorting. 
...
Bubble sort makes repeated passes over the data. On each pass it
compares every adjacent pair of elements and exchanges them if they
are out of order. The largest remaining value therefore "bubbles"
all the way to the end of the region being scanned.

After p complete passes, the last p elements are already in their
final positions, so pass p+1 can stop p elements earlier. That is
what the "n - 1 - pass" bound does.

Cutting off happens if a complete pass makes zero exchanges, then every adjacent
pair was already in order, which means the whole array is sorted and stopping
immediately occurs. 
This is what makes Bubble Sort O(n) on data that is already sorted. */

void sortarray (int a[], int n) {

    int exchanges = 1;          // start at 1 so the loop runs at least once
    int pass = 0;               // how many complete passes we have made
